...
namespace BeWarehouseHub.Core.Helpers.Pdf;

public static class PdfExportHelper
{
    static PdfExportHelper() { }

    public static byte[] GenerateExportReceiptPdf(ExportReceiptDto receipt)
    {
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Column(col =>
                {
                    ...
                    col.Item()
                        .AlignCenter()
                        .Text($"Số chứng từ: {receipt.ExportId.ToString()[..8].ToUpper()}");

                    col.Item()
                        .PaddingVertical(5)
                        .Row(row =>
                        {
                            row.RelativeItem().Text($"Ngày xuất: {receipt.ExportDate:dd/MM/yyyy HH:mm}");
                            row.RelativeItem().AlignRight().Text($"Kho xuất: {receipt.WarehouseName}");
                        });
                        ...
                        .Column(box =>
                        {
                            box.Item().Table(table =>
                            {
                                ...
                                foreach (var (item, index) in receipt.Details.Select((x, i) => (x, i)))
                                {
                                    ...
                                    table.Cell().Element(CellStyle).AlignRight().Text(item.Amount.ToString("N0"));
                                }
                                ...
                                table.Cell()
                                    .Element(CellStyle)
                                    .AlignCenter()
                                    .Text(receipt.TotalQuantity.ToString("N0"))
                                    .Bold();
                                ...
                                table.Cell()
                                    .Element(CellStyle)
                                    .AlignRight()
                                    .Text(receipt.TotalAmount.ToString("N0"))
                                    .Bold();
                            });
                        });
                });
